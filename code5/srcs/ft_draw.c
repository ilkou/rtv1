...
void            ft_cadre(t_ptr *p)
{
    ...
    t_objects obj2;
    t_objects obj3;
    t_sphere    s1 = ft_sphere(ft_vector(0.6,0,-1), 0.6001);
    t_sphere    s2 = ft_sphere(ft_vector(-0.6,0,-1), 0.6001);
    ...
    while (--row >= 0)
    {
        ...
        while (++col < WIN_WIDTH)
        {
            ...
            while (++omg < 100)
            {
                float u = (double)(col + (rand() / 1.0)) / WIN_WIDTH;
                float v = (double)(row + (rand() / 1.0)) / WIN_HEIGHT;
