...
{
	return (ft_add(r->origin, ft_pro_k(r->dir, t))); //A+t*B
}
...
{
    u = (double)(u + myrand48()) / IMG_WIDTH;
    v = (double)(v + myrand48()) / IMG_HEIGHT;
    return (ft_ray(p->origin, ft_normalize(ft_sub(ft_add(p->lower_left_corner,
            ft_add(ft_pro_k(p->horizontal, u), ft_pro_k(p->vertical, v))),
                    p->origin))));
}