...
int     ft_scatter_dielectric(double ref_idx, t_ray *r, t_hit *rec, t_vector *att, t_ray *scattered)
{
    ...
    *att = ft_vector(1.0, 1.0, 1.0);
    ...
    ni_over_nt = (ft_dot(r->B, rec->normal) > 0) ? ref_idx : 1.0 / ref_idx;
    cos = (ft_dot(r->B, rec->normal) > 0) ? ref_idx * ft_dot(r->B, rec->normal)
            / ft_length(r->B) : - ft_dot(r->B, rec->normal) / ft_length(r->B);
    reflect_prob = (ft_refract(&r->B, &outward_normal, ni_over_nt, &refracted))
            ? ft_schlick(cos, ref_idx) : 1.0;
...
t_vector    ft_color(t_ray *r, t_ptr *p, t_objects *o, int depth)
{
    ...
    if (depth < 50 && ft_hit(o, r, ft_interval(0.001, DBL_MAX), &rec))
    {
        t_ray       scattered;
        t_vector    att;
        if ((rec.nbr == 4 || rec.nbr == 5) && ft_scatter_dielectric(rec.ref_idx, r, &rec, &att, &scattered))
            return (ft_pro(att, ft_color(&scattered, p, o, depth+1)));
        else if (rec.nbr == 3 && ft_scatter_metal(*rec.material, r, &rec, &att, &scattered))
            return (ft_pro(att, ft_color(&scattered, p, o, depth+1)));
...
void            *ft_calcul(t_thread *p)
{
    ...
    t_objects obj4;
    t_objects obj5;
    t_sphere    s1 = ft_sphere(ft_vector(0,0,p->e->cc), 0.5, ft_vector(0.1, 0.2, 0.5));
    t_sphere    s2 = ft_sphere(ft_vector(0,-100.5,-1) , 100, ft_vector(0.8, 0.8, 0.0));
    t_sphere    s3 = ft_sphere(ft_vector(1,0,-1), 0.5, ft_vector(0.8, 0.6, 0.2));
    t_sphere    s4 = ft_sphere_dielec(ft_vector(-1,0,-1), 0.5, 1.5);
    t_sphere    s5 = ft_sphere_dielec(ft_vector(-1,0,-1), -0.45, 1.5);
